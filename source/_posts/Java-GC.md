...
**年轻代**：大多数新创建的对象被分配到这里。因为大多数对象很快变得不可达，所以很多对象在年轻代中创建，然后消失。当对象人家个区域消失时，我们说一个"**minor GC**"发生了。
...
如果你想要理解G1收集器，首先你要忘记你所理解的年轻代和年老代。正如你在上图所看到的，每个对象被分配到不同的风格中，随后执行垃圾回收。当一个区域填满之后，对象被转移到另一个区域，并再执行一次垃圾回收。在这种垃圾回收算法中，不再有从年轻代移动到年老代的三部曲。这个类型的垃圾收集算法是为替代CMS收集器而被创建的，因为CMS收集器在长时间持续运行时会产生很多问题。
